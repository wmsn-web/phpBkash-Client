<?php
require '../vendor/autoload.php';
use WmsnWeb\BkashPhpClient\BkashPhpClient;

$credentials = array(
	"base_url" => "https://tokenized.sandbox.bka.sh/v1.2.0-beta/tokenized/",
	"username" => "sandboxTokenizedUser02",
	"password" => "sandboxTokenizedUser02@12345",
	"app_key" => "4f6o0cjiki2rfm34kfdadl1eqq",
	"app_secret" => "2is7hdktrekvrbljjh44ll3d9l1dtjo4pasmjvs5vl5qr3fug4b",
	"type"		=>"sandbox"
);
$bkash = new BkashPhpClient($credentials);

//Here is the saved refresh token

$token = "eyJraWQiOiJvTVJzNU9ZY0wrUnRXQ2o3ZEJtdlc5VDBEcytrckw5M1NzY0VqUzlERXVzPSIsImFsZyI6IlJTMjU2In0..qI_HT2uzqAeUdxdeK48Bl8ZSGduiJ62dElc-OR4o9-vpexoReZggVLcheHKApL3ML42fhVLRXE8oBP2iyMpdKvzPlEW56aP--7bfxHJCmq-n0jcKNCfxQoGFnN1P9pUIZPBhz45uOzRkpjOrLVhGDSwRFepeTirNPikfTLPXpnO5ZgZLyevRdqnRfSVD-INCjRMwTCzppPfz6nM5QzrvaN-7PB2f_XtgIkofyMTKLVucwrl4E27Pj-IT33FSyZ3kvpgHlPcami0C837td2gVkMCSkJD_9tgjTe7A1yzajqJXsZ0sqrmpFMHmgAUHHaKcXRg1WCJ5Dg4jz_zLkXu2vA";

$invoice_id = 'Inv'.rand();
$postArray = array(
    'mode' => '0011',
    'amount' => 100,
    'payerReference' =>"11",
    'callbackURL' => 'http://localhost/test_composer/BkashPhpClient/examples/execute_payment.php?inv='.$invoice_id, // Your callback URL
    'currency' => 'BDT',
    'intent' => 'sale',
    'merchantInvoiceNumber' => $invoice_id
);

$response = $bkash->createPayment($token, $postArray);
echo "<pre>";
print_r($response);

//Example Output
/*
Array
(
    [paymentID] => TR0011gfMBThn1751047754935
    [bkashURL] => https://sandbox.payment.bkash.com/?paymentId=TR0011gfMBThn1751047754935&hash=.0!Kv_sT5(YOvZ7-RdDEP69dDvT*bVGZTh(sKYMAJ1nK_lsn9ysmQtAPA.dlgHLSDx7DyqnF-3Ri.ZI-myTM5-jIgTgL9I!PUEH41751047754935&mode=0011&apiVersion=v1.2.0-beta/
    [callbackURL] => http://localhost/test_composer/BkashPhpClient/examples/execute_payment.php?inv=Inv790446684
    [successCallbackURL] => http://localhost/test_composer/BkashPhpClient/examples/execute_payment.php?inv=Inv790446684&paymentID=TR0011gfMBThn1751047754935&status=success&signature=Hq9vQ5KxiH
    [failureCallbackURL] => http://localhost/test_composer/BkashPhpClient/examples/execute_payment.php?inv=Inv790446684&paymentID=TR0011gfMBThn1751047754935&status=failure&signature=Hq9vQ5KxiH
    [cancelledCallbackURL] => http://localhost/test_composer/BkashPhpClient/examples/execute_payment.php?inv=Inv790446684&paymentID=TR0011gfMBThn1751047754935&status=cancel&signature=Hq9vQ5KxiH
    [amount] => 100
    [intent] => sale
    [currency] => BDT
    [paymentCreateTime] => 2025-06-28T00:09:14:935 GMT+0600
    [transactionStatus] => Initiated
    [merchantInvoiceNumber] => Inv790446684
    [statusCode] => 0000
    [statusMessage] => Successful
)
*/